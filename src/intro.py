...
def banner_sparkle(short: bool, screen: Surface) -> None:
    ...
    for banner in os.listdir(join(directories.IMAGES_DIR, 'intro_banner', 'sparkle')):
        before_frame = calculation.convert_to_frames(get_ticks())
        if short:
            frames_per_slide = 1.5
        else:
            frames_per_slide = 3
        while calculation.convert_to_frames(get_ticks()) < before_frame + frames_per_slide:
            intro_banner_rect = show_intro_banner(join(directories.IMAGES_DIR, 'intro_banner', 'sparkle', banner), screen, False)
...
def draw_banner_text(screen: Surface, config: dict):
    ...
    draw_text("(↑ ← ↓ →)", screen.get_width() / 2, screen.get_height() * 15 / 16, screen, config, PINK,
              font_name=directories.SMB_FONT_PATH, alignment='center', letter_by_letter=False)


def repeated_sparkle(screen: Surface, clock_check, short, ticks) -> int | float:
    if ticks - clock_check >= calculation.convert_to_milliseconds(256):
        clock_check = ticks
...
class Intro:
    ...
    def show_start_screen(self, screen, start_time, clock, config):
        ...
        directories = Directories(config)
        calculation = Calculation(config)
        intro_banner_rect = show_intro_banner(directories.INTRO_BANNER_PATH, screen, config['NO_BLIT'])
        display.update(intro_banner_rect)
        waiting = True
        while waiting:
            ...
            for current_event in event.get():
                if current_event.type == QUIT:
                    ...
                    if current_event.key in accept_keys:
                        waiting = False
            if calculation.convert_to_frames_since_start_time(start_time) >= 620:  # intro banner with text displays 620 frames in
                waiting = False
            display.update(intro_banner_rect)
    ...
    def show_intro_dragon_banner_with_text(self, screen, clock, config):
        show_intro_banner(directories.INTRO_BANNER_WITH_DRAGON_PATH, screen, config['NO_BLIT'])
    ...
    def handle_all_sparkles(self, start_time, screen):
        frames_since_banner_launch = calculation.convert_to_frames_since_start_time(start_time)
