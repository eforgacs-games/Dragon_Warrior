...
def draw_stats_strings_with_alignments(stat_string, y_position, screen):
    if len(stat_string) > 4:
        draw_text(stat_string, TILE_SIZE * 3.2, TILE_SIZE * y_position, screen, alignment='center')
    elif len(stat_string) > 3:
        draw_text(stat_string, TILE_SIZE * 3.44, TILE_SIZE * y_position, screen, alignment='center')
    elif len(stat_string) > 2:
        draw_text(stat_string, TILE_SIZE * 3.67, TILE_SIZE * y_position, screen, alignment='center')
    elif len(stat_string) > 1:
        draw_text(stat_string, TILE_SIZE * 3.99, TILE_SIZE * y_position, screen, alignment='center')
    else:
        draw_text(stat_string, TILE_SIZE * 4.2, TILE_SIZE * y_position, screen, alignment='center')


def draw_hovering_stats_window(screen, player):
    create_window(1, 2, 4, 6, HOVERING_STATS_BACKGROUND_PATH, screen)
    draw_text(player.name[:4], TILE_SIZE * 2.99, TILE_SIZE * 2, screen, alignment='center')
